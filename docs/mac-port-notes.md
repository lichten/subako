...
§2.2 は `WindowLeft/Top/Width/Height` `WindowMaximized` `SidebarWidth` を
永続化対象としているが、**Mac 版でアプリが持つ必要があるのはサイドバー幅だけ**。
実測で確かめた macOS の挙動:

- **位置とサイズは SwiftUI の `WindowGroup` が既に保存・復元している**。
  AppKit の frame autosave が働き、UserDefaults の
  `NSWindow Frame SubakoApp.MainWindow-…` に**移動・リサイズのたびに**書かれる。
  終了時にまとめて書く方式と違い、強制終了でも取りこぼさない。
- **画面外からの復帰も macOS がやる**。保存フレームを `-5000,-5000` や、
  単一ディスプレイ環境で `x=3000`(右の外部ディスプレイを外した想定)にして
  起動しても、macOS が画面内へ戻した。自前のクランプ処理は発火しなかったので入れていない。
  ただし Apple のドキュメント上 `constrainFrameRect(_:to:)` が保証するのは
  「上端を画面に乗せる」「高さを縮める」だけで**横方向は保証外**なので、
  将来おかしくなったらここを疑うこと。
