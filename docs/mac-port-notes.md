...
- **画面外対策は自前で必要**。macOS が自動で行う `constrainFrameRect(_:to:)` は
  ...
  (Apple ドキュメント明記、実測でも同じ)。`WindowPlacement.onScreen` で対応する:
  - 保存先ディスプレイが無い → 主ディスプレイの中央へ
  - **画面端をまたいだまま終了した → 重なりの大きい画面の中へ寄せる**。
    これが無いと、例えば画面幅と同じ幅のウィンドウを x=300 に置いて終了した場合、
    縦だけ補正されて横ははみ出したまま復元され、毎回はみ出して開くようになる
    (実測: 300,17,1710,990 → 0,84,1710,989 に補正)。
  - 画面より大きい矩形は縮める
