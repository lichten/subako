...
### 1.7 フォロー中一覧の受け渡し (`data/_followings/<source>.jsonl`)

ビューアの「フォロー中を一括登録」が `python main.py <source> --followings` を
起動し、fetcher が `/follows` の結果をここへ書き、ビューアがプロセス終了後に
読んで `users` へまとめて登録するための**受け渡しファイル**。

- 1 行 = `/follows` が返した User オブジェクトの生データ (UTF-8、`\n` 終端)。
  §2 と同じ「生のまま 1 行 1 件」規約だが、**追記ではなく実行ごとの全書き換え**
  (fetcher は開始時に既存ファイルを削除し、一時ファイルに書いてから rename する)。
- ビューアが使うのは `username` (必須) と `display_name` (任意) だけ。
  壊れた行・`username` の無い行はスキップし、handle は大文字小文字無視で重複排除する。
- **`icon_url` は users に入れない**。数千件を一度に登録するとアイコンキャッシュ
  (§3.5) が同数の HTTP 取得を一斉に始めてしまうため。表示名・アイコンは
  そのユーザーのツイートを取得した時点で §5 の規則で埋まる。
...
- **`/follows` はページングの終端を `next_cursor: "0"` (文字列) で表す**。null でも
  不在でもないので、真偽値での判定はできない (Python の `"0"` は真)。しかも `"0"` を
  送ると 1 ページ目が返り続けるため、素朴に書くと同じページを取り続けて
  リクエスト上限まで走る。`/user-tweets` は終端で偽値のカーソルを返す別の流儀なので
  混同しないこと。fetcher は「終端カーソル」「新規 0 件のページ」「カーソルが前回と
  同一」の 3 条件で打ち切る (`main.collect_followings`、`tests/test_followings.py`)。
- 中断 (プロセス kill) された実行はファイルを残さない。リクエスト上限 (exit 10)
  や API エラーで途中終了した場合は、そこまでに取れた分を書き出す。
  カーソルの保存はしない (1 ページ 200 件で取り直しが安く、フォロー一覧は
  追記型の履歴ではなく変動する集合のため、時間差のページを継ぐと重複と欠落が出る)。
