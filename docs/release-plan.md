...
**MIT を推奨**。理由: 依存がすべて MIT / Apache-2.0 / WTFPL / BSD-3 で整合し、個人 OSS の事実上の標準で、利用側の心理的障壁が最も低い。コピーレフト (GPL 系) にする場合は、依存には問題ないが、フォーク・派生の敷居が上がることだけ理解して選ぶ。

### 0-3. リポジトリの扱いの決定 (S)

- [ ] 「現リポジトリを rename して public 化」か「新リポジトリへ移す」かを決める

判断材料:

- git 履歴に**秘密情報 (API キー・.env・data/・probe_output/) は無い**ことを監査で確認済み (`git log --all --diff-filter=A --name-only` と キー断片の `git log -S` で検証)
- ただし**実在ハンドル入りのコメント・テスト fixture は履歴に残る** (フェーズ 1-3 で現ファイルから消しても、過去コミットには残る)。「履歴ごと公開してよいか」を判断すること
- 履歴を捨てて新リポジトリに squash 移行すれば完全にクリーンになるが、36 コミットの開発履歴 (コミットメッセージに設計判断が書かれている) を失う
...
- [ ] リポジトリ直下に `LICENSE` を置く

現状 **LICENSE が存在しない** = 法的には all rights reserved で、GitHub に置いても他人はフォークも再配布もできない。OSS 公開の必須要件。
